...
// floating inputs (even with INPUT_PULLPUP). Note that all the data structures, etc
// are still set up for 12 bells, as the simulators send and expect 12-bell data.
// This is just so we ignore unused inputs.
int numChannels;

// Define the default number of channels in case the EEPROM is not yet set sensibly.
// There is no minNumChannels as this will be 1.
const int defaultNumChannels = 12; // 1 for the one-bell, 12 for the 12-bell

// Define the maximum number of channels that can be set. For tthe one-bell interface this
// is 1. For the 12-bell version this will be 12.
...
int bellSensorLastValue[12]; //initialise these in setup, and set them LOW (see below)
...
unsigned long bellCharSendTime[12]; // initialise these in setup()
...
int serialSpeed = 0; // index into serialSpeeds[], not the actual bps 
...
unsigned long bellDebounceEndTime[12]; // initialise these in setup()
...
// program state "WAIT_FOR_INPUT" (0), "WAIT_FOR_DEBOUNCE" (1), "WAIT_TO_SEND"(2), or
// "TEST_MODE" (3))
int bellMachineState[12]; // initialise these in setup()
...
// Set up a Debug Mask variable. The first twelve bits (0-11) correspond to debugging
// being enabled on that channel (1=true=on, 0=false=off). This allows per-channel debugging
// to be configured. LSB = Channel 0 (usually treble).
int debugBellMask = 4095; // first 12 bits all on (true) by default. See option M.
...
unsigned long pulseStartTime[12]; // initialise these in setup()
unsigned long pulseEndTime[12];   // initialise these in setup()
...
int pulseTimeLastValue[12]; //initialise these in setup, and set them LOW (see below)
...
int pulseTimeCount[12]; //initialise in setup, and set to zero
...
void setup() {   
	...
	// setup data structures - all 12 values here and not numChannels
	for ( i = 0; i < 12; i++ ) {
		...
		// initialize the state machine flag for each bell. All start by waiting for input.
		// Setting this to "WAIT_FOR_INPUT" would result in an immediate timer expiry for every
	...
	// initialise the number of active channels from EEPROM. If the value is <1 or >12, set
	// it to 12. Cast the byte value read into an int.
	numChannels = int( EEPROM.read(18) );
	// but the EEPROM value may not be set to a sensible value! Seed a sane value.
	if ( numChannels < 1 || numChannels > 12 ) {
